...
function getPlanets(dob, tob, pobStr) {
  ...
  if (hourUT >= 24  { name: "Sun", symbol: "☉", color: "#FFD700", glyph: "Su" },
  { name: "Moon", symbol: "☽", color: "#C8C8D4", glyph: "Mo" },
  { name: "Mars", symbol: "♂", color: "#FF6B4A", glyph: "Ma" },
  { name: "Mercury", symbol: "☿", color: "#6EE7B7", glyph: "Me" },
  { name: "Jupiter", symbol: "♃", color: "#FFA040", glyph: "Ju" },
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      ...
                      <span style={{background:`${C.gold}22`,color:C.gold,fontSize:9,
                        padding:"1px 5px",borderRadius:4}}>Pada {p.pada}</span>
                      <span style={{color:C.muted,fontSize:9}}>Lord: {p.nakLord}</span>
                    </div>
                  </div>
                  {/* House badge */}
...
function RemediesScreen({profile}) {
  ...
  const fetchAIR=async()=>{
    if(!profile?.dob)return;
    setAiLoading(true);setAiRemedy(null);
    try{
      const r=await fetchAIRemedy({name:profile.name,zodiac:profile.zodiac||ZODIAC[getZodiacIdx(profile.dob)],area});
      setAiRemedy(r);
    }catch(e){setAiRemedy(null);}
    setAiLoading(false);
  };

  useEffect(()=>{setActive(null);setAiRemedy(null);},[area]);

  const tap=()=>{
    if(!active||done)return;
    const n=count+1;setCount(n);
    if(n>=active.count)setDone(true);
  };

  const remedies=REMEDIES[area]||REMEDIES["Career"];
  const allRemedies=aiRemedy?[...remedies,{...aiRemedy,isAI:true}]:remedies;

  return (
    <div style={{padding:"20px",maxWidth:520,margin:"0 auto",animation:"fadeUp 0.5s ease"}}>
      <h2 style={{fontFamily:"'Cinzel',serif",color:C.gold,margin:"0 0 4px",fontSize:22}}>🕉️ Vedic Remedies</h2>
      <p style={{color:C.muted,fontSize:13,marginBottom:20,fontFamily:"'Crimson Pro',serif",fontSize:15}}>Scriptural mantras & AI-personalised rituals</p>

      {/* Area pills */}
      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:18}}>
        {LIFE_AREAS.map(a=>(
          <button key={a} className="pill" onClick={()=>setArea(a)} style={{
            padding:"5px 12px",border:`1px solid ${area===a?C.gold:C.border}`,
            borderRadius:20,cursor:"pointer",fontSize:11,transition:"all 0.2s",
            background:area===a?`${C.gold}22`:C.card,
            color:area===a?C.gold:C.muted,fontWeight:area===a?700:400,
          }}>{a}</button>
        ))}
      </div>
      ...
      {!active?(
        <div>
          {/* AI Remedy Button */}
          {profile?.dob&&(
            <button onClick={fetchAIR} disabled={aiLoading} style={{
              width:"100%",padding:12,border:`1px solid ${C.purple}`,borderRadius:12,cursor:"pointer",
              background:`linear-gradient(135deg,${C.purple}22,${C.card})`,
              color:C.purpleLight,fontSize:13,fontWeight:600,marginBottom:14,
              display:"flex",alignItems:"center",justifyContent:"center",gap:8,
              opacity:aiLoading?0.7:1,transition:"all 0.3s",
            }}>
              {aiLoading?<><div style={{width:14,height:14,border:`2px solid ${C.purple}`,borderTopColor:"transparent",borderRadius:"50%",animation:"spin 0.7s linear infinite"}}/> Generating AI Remedy...</>:"🤖 Generate AI Personalised Remedy"}
            </button>
          )}

          {allRemedies.map((r,i)=>(
            <div key={i} className="card-hover" style={{
              background:r.isAI?`linear-gradient(135deg,${C.purple}22,${C.card})`:C.card,
              border:`1px solid ${r.isAI?C.purple:C.border}`,
              borderRadius:16,padding:18,marginBottom:12,cursor:"pointer",
              transition:"all 0.3s",animation:`fadeUp 0.4s ease ${i*0.07}s both`,
            }} onClick={()=>{setActive(r);setCount(0);setDone(false);}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                <div style={{fontFamily:"'Cinzel',serif",color:r.isAI?C.purpleLight:C.gold,fontSize:14,fontWeight:700,flex:1}}>{r.name}</div>
                <div style={{display:"flex",gap:6,marginLeft:8}}>
                  {r.isAI&&<div style={{background:`${C.purple}33`,color:C.purpleLight,fontSize:9,padding:"2px 7px",borderRadius:10}}>AI</div>}
                  <div style={{background:`${C.gold}22`,color:C.gold,fontSize:9,padding:"2px 7px",borderRadius:10}}>{r.count}×</div>
                </div>
              </div>
              <div style={{color:C.muted,fontSize:12,fontStyle:"italic",marginBottom:8,fontFamily:"'Crimson Pro',serif",lineHeight:1.6}}>"{r.mantra.slice(0,60)}{r.mantra.length>60?"...":""}"</div>
              <div style={{display:"flex",justifyContent:"space-between"}}>
                <span style={{color:C.muted,fontSize:11}}>🪷 {r.deity}</span>
                <span style={{color:C.muted,fontSize:11}}>✦ {r.benefit}</span>
              </div>
              {r.instructions&&<div style={{marginTop:10,background:`${C.purple}22`,borderRadius:8,padding:8,color:C.purpleLight,fontSize:11,lineHeight:1.5}}>{r.instructions}</div>}
              <div style={{marginTop:10,padding:8,background:`${C.gold}11`,borderRadius:8,textAlign:"center",color:C.gold,fontSize:12,fontWeight:600}}>▶ Begin Japa</div>
            </div>
          ))}
        </div>
      ):(
        <div style={{animation:"fadeUp 0.4s ease",textAlign:"center"}}>
          <button onClick={()=>setActive(null)} style={{background:"transparent",border:`1px solid ${C.border}`,color:C.muted,padding:"6px 16px",borderRadius:8,cursor:"pointer",fontSize:12,marginBottom:20}}>← Back</button>

          <div style={{background:C.card,border:`1px solid ${C.gold}44`,borderRadius:20,padding:24,marginBottom:16}}>
            <div style={{fontFamily:"'Cinzel',serif",color:C.gold,fontSize:17,marginBottom:6}}>{active.name}</div>
            <div style={{color:C.purpleLight,fontSize:14,fontStyle:"italic",lineHeight:1.8,marginBottom:14,fontFamily:"'Crimson Pro',serif"}}>"{active.mantra}"</div>
            <div style={{color:C.muted,fontSize:12,marginBottom:20}}>🪷 {active.deity} · Chant {active.count} times</div>

            {/* Mala circle */}
            <div style={{position:"relative",marginBottom:16}}>
              <div style={{
                width:160,height:160,borderRadius:"50%",margin:"0 auto",
                background:done?`linear-gradient(135deg,${C.gold},${C.saffron})`:`radial-gradient(circle at 35% 35%,${C.purple}66,${C.surface})`,
                border:`3px solid ${done?C.gold:C.purple}66`,
                display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
                cursor:done?"default":"pointer",transition:"all 0.3s",
                boxShadow:done?`0 0 50px ${C.gold}66`:`0 0 20px ${C.purple}33`,
                animation:!done?"pulse 2.5s ease-in-out infinite":"none",
              }} onClick={tap}>
                <div style={{fontSize:done?36:30,marginBottom:2}}>{done?"🙏":"🕉️"}</div>
                <div style={{fontFamily:"'Cinzel',serif",fontSize:32,color:done?"#fff":C.gold,fontWeight:700,lineHeight:1}}>{count}</div>
                <div style={{color:done?"#ffffffaa":C.muted,fontSize:11,marginTop:2}}>of {active.count}</div>
              </div>
            </div>

            {/* Progress */}
            <div style={{background:C.surface,borderRadius:10,height:6,marginBottom:12,overflow:"hidden"}}>
              <div style={{height:"100%",borderRadius:10,transition:"width 0.3s ease",width:`${Math.min((count/active.count)*100,100)}%`,background:`linear-gradient(90deg,${C.purple},${C.gold})`}}/>
            </div>

            {done?(
              <div style={{color:C.gold,fontFamily:"'Cinzel',serif",fontSize:16}}>
                ✦ Sadhana Complete ✦<br/>
                <span style={{fontSize:12,color:C.muted,fontFamily:"sans-serif"}}>Om Shanti Shanti Shantihi 🙏</span>
              </div>
            ):(
              <div style={{color:C.muted,fontSize:13}}>{active.count-count} remaining · Tap the circle to count</div>
            )}
          </div>

          <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:16,textAlign:"left"}}>
            <div style={{color:C.gold,fontSize:13,fontWeight:700,marginBottom:10,fontFamily:"'Cinzel',serif"}}>📿 How to Perform</div>
            {["Sit facing East or North","Light incense or a diya if possible","Close eyes, take 3 deep breaths","Chant clearly with full devotion","Offer gratitude when complete"].map((s,i)=>(
